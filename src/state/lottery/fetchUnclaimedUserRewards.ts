...
const fetchCakeRewardsForTickets = async (
  ...
  try {
    ...
    const cakeTotal = cakeRewards.reduce((accum: BigNumber, blhReward: ethers.BigNumber[]) => {
      return accum.plus(new BigNumber(blhReward[0].toString()))
    }, BIG_ZERO)

    const ticketsWithUnclaimedRewards = winningTickets.map((winningTicket, index) => {
      return { ...winningTicket, blhReward: cakeRewards[index] }
    })
